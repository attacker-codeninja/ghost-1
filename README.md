...
ghost_version=$(git rev-parse --abbrev-ref HEAD)-$(git describe --tags)
...
AXIOM_FLEET_NAME="ghost"
AXIOM_FLEET_COUNT=10
AXIOM_FLEET_REGIONS="eu-central"
AXIOM_FLEET_SHUTDOWN=true
# This is a script on your ghost host that might prep things your way...
...
```bash
./ghost.sh -l sites.txt -r -o /output/directory/
```
...
```bash
./ghost.sh -d target.com -r --deep -o /output/directory/
```
 
**Perform recon in a multi domain target**
 
```bash
./ghost.sh -m company -l domains_list.txt -r
```

**Perform recon with axiom integration**
 
```bash
./ghost.sh -d target.com -r -v
```
 
 **Perform all steps (whole recon + all attacks) a.k.a. YOLO mode**
 
```bash
./ghost.sh -d target.com -a
```
...
- Url extraction ([waybackurls](https://github.com/tomnomnom/waybackurls), [gauplus](https://github.com/bp0lr/gauplus), [gospider](https://github.com/jaeles-project/gospider), [github-endpoints](https://gist.github.com/prakashzhaa/d1d516b606557526e9a78d7dd49cacd3) and [JSA](https://github.com/w9w/JSA))
...
- Docker container included and [DockerHub](https://hub.docker.com/r/prakashzhaa/ghost) integration
...
* Clone your project: `git clone https://gitlab.com/example/ghost-data`
* Get inside the cloned repository: `cd ghost-data`
* Create branch with an empty commit: `git commit --allow-empty -m "Empty commit"`
* Add official repo as a new remote: `git remote add upstream https://github.com/prakashzhaa/ghost` (`upstream` is an example)
