...
printf "\n\n${bgreen}#######################################################################${reset}\n"
printf "${bgreen} ghost installer/updater script ${reset}\n\n"
...
if [[ $(id -u | grep -o '^0$') == "0" ]]; then
    ...
    if sudo -n false 2>/dev/null; then
        ...
        printf "${bred} echo \"${USERNAME}  ALL=(ALL:ALL) NOPASSWD: ALL\" > /etc/sudoers.d/ghost${reset}\n\n"
    fi
    SUDO="sudo"
fi
...
printf "${bblue} Running: Looking for new ghost version${reset}\n\n"
...
then
    ...
    if [ -n "$(git status --porcelain | egrep 'ghost.cfg$')" ]; then
        mv ghost.cfg ghost.cfg_bck
        printf "${yellow} ghost.cfg has been backed up in ghost.cfg_bck${reset}\n\n"
    fi
    eval git reset --hard $DEBUG_STD
    eval git pull $DEBUG_STD
    printf "${bgreen} Updated! Running new installer version...${reset}\n\n"
else
    printf "${bgreen} ghost is already up to date!${reset}\n\n"
fi
...
eval wget -nc -O ~/.config/notify/provider-config.yaml https://gist.githubusercontent.com/prakashzhaa/23a996bca189a11e88251367e6583053/raw $DEBUG_STD
...
eval wget -O resolvers_trusted.txt https://gist.githubusercontent.com/prakashzhaa/ae9ed7e5c786461868abd3f2344401b6/raw $DEBUG_STD
eval wget -O subdomains.txt https://gist.github.com/prakashzhaa/a307a04a222fab5a57466c51e1569acf/raw $DEBUG_STD
eval wget -O permutations_list.txt https://gist.github.com/prakashzhaa/ffc2b14d283e8f8eff6ac83e20a3c4b4/raw $DEBUG_STD
eval wget -nc -O fuzz_wordlist.txt https://raw.githubusercontent.com/prakashzhaa/OneListForAll/main/onelistforallmicro.txt $DEBUG_STD
eval wget -O lfi_wordlist.txt https://gist.githubusercontent.com/prakashzhaa/a89a0c7861d49bb61a09822d272d5395/raw $DEBUG_STD
eval wget -O ssti_wordlist.txt https://gist.githubusercontent.com/prakashzhaa/ab5277b11da7369bf4e9db72b49ad3c1/raw $DEBUG_STD
eval wget -O headers_inject.txt https://gist.github.com/prakashzhaa/d62ab8f8ffd28e1c206d401081d977ae/raw $DEBUG_STD
eval wget -O custom_udork.txt https://gist.githubusercontent.com/prakashzhaa/7245cad74f2da5824080e0cb6bdaac22/raw $DEBUG_STD
eval wget -O axiom_config.sh https://gist.githubusercontent.com/prakashzhaa/6e2d9f4932fd38d84610eb851014b26e/raw $DEBUG_STD
...
printf "${yellow} Remember set your api keys:\n - amass (~/.config/amass/config.ini)\n - subfinder (~/.config/subfinder/config.yaml)\n - GitHub (~/Tools/.github_tokens)\n - SHODAN (SHODAN_API_KEY in ghost.cfg or env var)\n - SSRF Server (COLLAB_SERVER in ghost.cfg or env var) \n - Blind XSS Server (XSS_SERVER in ghost.cfg or env var) \n - notify (~/.config/notify/provider-config.yaml) \n - theHarvester (~/Tools/theHarvester/api-keys.yml)\n - H8mail (~/Tools/h8mail_config.ini)\n - uDork FB cookie (UDORK_COOKIE in ghost.cfg or env var)\n - WHOISXML API (WHOISXML_API in ghost.cfg or env var)\n\n\n${reset}"
