...
function google_dorks(){
	if [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] && [ "$GOOGLE_DORKS" = true ] && [ "$OSINT" = true ]; then
		...
		if [ "$GOOGLE_DORKS" = false ] || [ "$OSINT" = false ]; then
			printf "\n${yellow} ${FUNCNAME[0]} skipped in this mode or defined in ghost.cfg ${reset}\n"
		else
...
function sendToNotify {
	if [[ -z "$1" ]]; then
		...
		if [[ -n "$slack_channel" ]] && [[ -n "$slack_auth" ]]; then
			...
			curl -F file=@${1} -F "initial_comment=ghost zip file" -F channels=${slack_channel} -H "Authorization: Bearer ${slack_auth}" https://slack.com/api/files.upload &>/dev/null
		fi
	fi
}
...
function resolvers_update(){
	if [ "$update_resolvers" = true ]; then
		if [ ! "$AXIOM" = true ]; then	
			...
			notification "Updated\n" good
			axiom-exec 'wget -O /home/op/lists/resolvers_trusted.txt https://gist.githubusercontent.com/prakashzhaa/ae9ed7e5c786461868abd3f2344401b6/raw' &>/dev/null
			axiom-exec 'wget -O /home/op/lists/onelistforallmicro.txt https://raw.githubusercontent.com/prakashzhaa/OneListForAll/main/onelistforallmicro.txt' &>/dev/null
...
function ipcidr_target(){
	if [[ $1 =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9] ]]; then
		echo $1 | mapcidr -silent | anew -q target_ghost_ipcidr.txt
		if [ -s "./target_ghost_ipcidr.txt" ]; then 
			[ "$REVERSE_IP" = true ] && cat ./target_ghost_ipcidr.txt | dnsx -ptr -resp-only -silent -retry 3 | unfurl -u domains 2>/dev/null | sed 's/\.$//' | anew -q ./target_ghost_ipcidr.txt
			if [[ $(cat ./target_ghost_ipcidr.txt | wc -l) -eq 1 ]]; then
				domain=$(cat ./target_ghost_ipcidr.txt)
			elif [[ $(cat ./target_ghost_ipcidr.txt | wc -l) -gt 1 ]]; then
				unset domain
				list=${PWD}/target_ghost_ipcidr.txt
			fi
		fi
		if [ -n "$2" ]; then
			cat $list | anew -q $2
...
PROGARGS=$(getopt -o 'd:m:l:x:i:o:f:c:rspanwvh::' --long 'domain:,list:,recon,subdomains,passive,all,web,osint,deep,help,vps' -n 'ghost' -- "$@")
...
case $opt_mode in
        ...
        'w')
			if [ -n "$list" ]; then
				...
				printf "\n\n${bred} Web mode needs a website list file as target (./ghost.sh -l target.txt -w) ${reset}\n\n"
				exit
			fi
			webs_menu
			exit
